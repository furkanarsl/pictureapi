...
class LogService:
    ...
    async def get_logs_by_user(self, user_id: int, skip: int, limit: int) -> List[Log]:
        return await Log.filter(user_id=user_id).order_by('-query_date').limit(limit=limit).offset(offset=skip)
